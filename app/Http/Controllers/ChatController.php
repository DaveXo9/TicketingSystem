...
class ChatController extends Controller
{
    ...
    {
        $users = User::where('id', '!=', auth()->id())->latest()->filter(request(['userSearch']))->get();
        ...
        $messages = Message::where('user_id', auth()->id())->where('recepient_id', $recepient_id)
        ->orwhere('user_id', $recepient_id)->where('recepient_id', auth()->id())
        ->oldest()->filter(request(['search']))->get();
    ...
    public function show(User $recepient):View {
        ...
        $messages = Message::where('user_id', auth()->id())->where('recepient_id', $recepient_id)
        ->orwhere('user_id', $recepient_id)
        ->where('recepient_id', auth()->id())->oldest()->get();
